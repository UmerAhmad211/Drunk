...
Init::load_textures()
{
	const std::vector<std::string> card_base = {
		"assets/heart", "assets/diams", "assets/clubs", "assets/spade"
	};
	const std::vector<std::string> card_values = { "01", "02", "03", "04",
						       "05", "06", "07", "08",
						       "09", "10", "11", "12",
						       "13" };
	...
	for (size_t i = 0; i < CARDS_NO; ++i) {
		...
		size_t value = i % 13;
		std::string filename =
			card_base[suit] + card_values[value] + ".png";
...
void
Init::init_cards_state()
{
	int x = 150, spacing = 160, y = 258;

	for (size_t i = 24; i < CARDS_NO; ++i) {
		bool is_first_in_column = (i == 24 || i == 26 || i == 29 ||
					   i == 33 || i == 38 || i == 44 ||
					   i == 51);
