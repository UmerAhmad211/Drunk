...
Fndtion::draw_fnd()
{
	for (size_t i = 0; i < FND_NO; ++i) {
		if (!fnd_four[i].empty()) {
			DrawTexture(fnd_four[i].back().cards,
				    fnd_four[i].back().position.x,
...
void
Fndtion::update_fnd(const Cards_Props &cards)
{
...
bool
Fndtion::is_valid_move(const Cards_Props &card)
{
	int suit_i;
