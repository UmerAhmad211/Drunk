...
Cards_Props
deep_copy_card(const Cards_Props &orig)
{
...
Rectangle
set_rect(const std::string &card_type, const Fndtion &n_fnd)
{
	switch (card_type[7]) {
	case 'h':
...
bool
collision_checker(const Vector2 &mouse_pos, Vector2 &offset, Vector2 &old_pos,
		  bool &dragging, bool &loc_change, const Cards_Props &card)
{
...
void
set_old_pos(Vector2 &old_pos, bool &dragging, bool &loc_change,
	    const Cards_Props &card, Cards_Props &top_card, int y)
{
