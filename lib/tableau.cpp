...
Tableau::Tableau(Init *card_inst)
	: cards(card_inst)
{
	loc_change = true;
	dragging = false;
...
void
Tableau::tableau_move(Vector2 mouse_pos)
{
	bool ret_w = false;
	if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
		for (size_t i = 0; i < COL_NUM; ++i) {
			for (size_t j = 0; j < tableau_seven[i].size(); ++j) {
				float visi_height =
					(j == tableau_seven[i].size() - 1) ?
						static_cast<float>(T_HEIGHT) :
						27.0f;
				float card_height =
					tableau_seven[i][j].cards.height;
				tableau_seven[i][j].cards.height = visi_height;
				ret_w = collision_checker(mouse_pos, offset,
							  old_pos, dragging,
							  loc_change,
							  tableau_seven[i][j]);
		...
		for (size_t i = 0; i < s_size_cards; ++i) {
			set_old_pos(old_pos, dragging, loc_change,
				    tableau_seven[i_x][i_y + i], top_card, y);
			y += 27;
	...
	if (dragging && i_x >= 0 && i_x < COL_NUM && i_y >= 0 &&
		...
		int y = 0;
		for (size_t i = 0; i < s_size_cards; ++i) {
			update_pos(offset, tableau_seven[i_x][i_y + i],
				   top_card, y);
			tableau_seven[i_x][i_y + i] = top_card;
			y += 27;
		}
...
void
Tableau::move_cards_from_tab(Fndtion &n_fnd, int &points)
{
	if (dragging) {
		Rectangle card_rect = {
		...
		Rectangle fnd_rect =
			set_rect(tableau_seven[i_x][i_y].card_type, n_fnd);
...
bool
Tableau::check_valid_tab(Cards_Props frm_tab, Cards_Props to_tab)
{
...
void
Tableau::move_cards_from_tab_to_tab(int &points)
{
	if (dragging) {
		Rectangle card_rect = {
			tableau_seven[i_x][i_y].position.x,
		...
		for (size_t i = 0; i < COL_NUM; ++i) {
			if (tableau_seven[i].size() == 0 &&
				...
				Rectangle to_tab = { (float)(150 + 160 * i),
						     (float)(258), T_WIDTH,
						     T_HEIGHT };
			...
			for (size_t j = 0; j < tableau_seven[i].size(); ++j) {
				Rectangle tab_rect = { (float)(150 + 160 * i),
						       (float)(258 + 27 * j),
						       T_WIDTH, T_HEIGHT };
				if ((check_valid_tab(tableau_seven[i_x][i_y],
						     tableau_seven[i][j]) ||
...
bool
Tableau::card_moved_from_waste(Cards_Props &card_w, int &points)
{
...
void
Tableau::select_card_sequence(const size_t &col, const size_t &start_x)
{
	s_size_cards = 0;
...
void
Tableau::move_card(const Rectangle &to_tab, Cards_Props &card, const size_t &k,
		   const bool &add_y)
{
...
void
Tableau::emp_card_up(const Rectangle &to_tab, const size_t &i,
		     const bool &add_y)
{
